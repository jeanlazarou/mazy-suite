...
The Chain X-Ray panel shows the signal at every stage of the chain as stacked waveform lanes (red columns = 0 dBFS) with a playhead synced to playback. `engine.ProcessWithTaps` fires a callback after each enabled processor; `wasmProcessBufferStages` returns the processed audio plus per-stage bucketed min/max/RMS envelopes (computed Go-side — never ship full per-stage audio across the worker boundary). Processing while the panel is open always recaptures stages, so the lanes always match the processed buffer; `xrayStages` is cleared on track switch or a stage-less run. Opened from the transport bar (single track) or a per-row button in the album table (which activates that track).
