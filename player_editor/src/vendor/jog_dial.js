//https://github.com/ohsiwon/JogDial.js
//https://www.ohsean.net/plugins/jogdial/
/*
 * JogDial.js - v 1.0
 *
 ...
 */
export const JogDial = function (element, options) {
...
function setConstants() {
  ...
  JogDial.MobileEvent =
    ...
    window.navigator.userAgent.match(JogDial.MobileRegEx);
  JogDial.PointerEvent =
  ...
  JogDial.utils = {
    ...
    getCoordinates: function (e) {
      ...
      var target = e.target || e.srcElement,
        ...
        _x =
          (JogDial.MobileEvent ? e.targetTouches[0].clientX : e.clientX) -
          rect.left,
        _y =
          (JogDial.MobileEvent ? e.targetTouches[0].clientY : e.clientY) -
          rect.top;
      return { x: _x, y: _y };
    },
...
JogDial.Instance.prototype = {
  ...
  trigger: function triggerEvent(type, data) {
    switch (type) {
      ...
      default:
        window.alert(
          "Please Check your code:\njogDial does not have triggering event [" +
            type +
            "]"
        );
        break;
...
function setInstance(self, el, opt) {
  ...
  self.opt = JogDial.utils.extend(
    JogDial.utils.extend({}, JogDial.Defaults),
    opt
  );
  self.info = {} || self;
...
function setEvents(self) {
  ...
  function mouseDownEvent(e) {
    ...
    //Trigger down event
    if (self.pressed)
      JogDial.utils.triggerEvent(self.knob, JogDial.CustomEvent.MOUSE_DOWN);
  }
  ...
  function mouseDragEvent(e) {
    if (self.pressed) {
      ...
      info.now.rotation = JogDial.utils.getRotation(
        self,
        quadrant === undefined ? info.old.quadrant : quadrant,
        degree
      );
...
function angleTo(self, radian, triggeredDegree) {
  ...
  if (triggeredDegree) {
    // Update JogDial data information
    self.info.now = JogDial.utils.extend(
      {},
      { rotation: triggeredDegree, quadrant: quadrant }
    );
    self.info.old = JogDial.utils.extend(
      {},
      { rotation: triggeredDegree % 360, quadrant: quadrant }
    );
    JogDial.utils.extend(self.knob, {
