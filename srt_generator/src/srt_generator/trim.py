...
def trim_segments(audio, segments, threshold_db=-40.0, pad=0.12,
                  min_duration=0.2, max_gap=None, max_duration=None):
    ...
    max_duration caps how far past its start a segment is allowed to
    look for voice, so an over-long region ends at the last singing
    within the cap instead of being cut mid-word by the hard cap in
    srt_writer.postprocess.
    ...
    return _trim_to_voiced(
        voiced, segments, pad, min_duration, max_gap, max_duration
    )
...
def _trim_to_voiced(voiced, segments, pad, min_duration, max_gap=None,
    ...
    for start, end, text in segments:
        limit = end if not max_duration else min(end, start + max_duration)

        first_frame = max(int(start / FRAME), 0)
        last_frame = min(int(limit / FRAME) + 1, total)
        ...
        if indexes.size:
            ...
            trimmed_end = min(limit, (first_frame + last_voiced + 1) * FRAME + pad)
